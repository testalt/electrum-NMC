...
Gdk.threads_init()
APP_NAME = "Electrum-NMC"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from electrum_NMC.util import format_satoshis, parse_URI
from electrum_NMC.network import DEFAULT_SERVERS
...
def restore_create_dialog():
    ...
    dialog = Gtk.Dialog("electrum-NMC", parent=None, 
                        flags=Gtk.DialogFlags.MODAL,
...
def run_network_dialog( network, parent ):
    ...
    add_help_button(host_box, 'The name, port number and protocol of your Electrum-NMC server, separated by a colon. Example: "ecdsa.org:50002:s". Some servers allow you to connect through http (port 80) or https (port 443)')
    host_box.show()
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Electrum-NMC ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid namecoin address:\n"+to_address)
            return
        ...
        #@todo namecoin electrum-ltc modifies this...
