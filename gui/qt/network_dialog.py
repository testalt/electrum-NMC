...
from electrum_NMC import DEFAULT_SERVERS, DEFAULT_PORTS
...
class NetworkDialog(QDialog):
    def __init__(self, network, config, parent):
        ...
        msg = _("Electrum-NMC sends your wallet addresses to a single server, in order to receive your transaction history.") + "\n\n" \
            + _("In addition, Electrum-NMC connects to several nodes in order to download block headers and find out the longest blockchain.") + " " \
        ...
        msg = _("If auto-connect is enabled, Electrum-NMC will always use a server that is on the longest blockchain.") + " " \
            + _("If it is disabled, Electrum-NMC will warn you if your server is lagging.")
