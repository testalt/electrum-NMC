...
from electrum_NMC import Wallet, Wallet_2of2, Wallet_2of3
...
from electrum_NMC.mnemonic import prepare_seed

MSG_ENTER_ANYTHING    = _("Please enter a wallet seed, a master public key, a list of Namecoin addresses, or a list of private keys")
...
class InstallWizard(QDialog):
    ...
    def restore_or_create(self):
        vbox = QVBoxLayout()
        main_label = QLabel(_("Electrum-NMC could not find an existing wallet."))
        ...
        for i, (wtype,name) in enumerate(self.wallet_types):
            if not filter(lambda x:x[0]==wtype, electrum_NMC.wallet.wallet_types):
                continue
    ...
    def waiting_dialog(self, task, msg= _("Electrum-NMC is generating your addresses, please wait.")):
        def target():
            task()
