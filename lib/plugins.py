...
def init_plugins(self):
    ...
    if __builtin__.use_local_modules:
        ...
        imp.load_module('electrum_NMC_plugins', fp, pathname, description)
        plugin_modules = map(lambda name: imp.load_source('electrum_NMC_plugins.'+name, os.path.join(pathname,name+'.py')), plugin_names)
    else:
        import electrum_NMC_plugins
        plugin_names = [name for a, name, b in pkgutil.iter_modules(electrum_NMC_plugins.__path__)]
        plugin_modules = [ __import__('electrum_NMC_plugins.'+name, fromlist=['electrum_ltc_plugins']) for name in plugin_names]
